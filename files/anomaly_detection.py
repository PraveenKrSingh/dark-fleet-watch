...
def detect_loitering(df: pd.DataFrame, speed_kn: float = 1.5,
    ...
    for mmsi, group in df.sort_values("timestamp").groupby("mmsi"):
        ...
        is_slow = group["speed"] <= speed_kn
        if not is_slow.any():
            continue
        # find contiguous slow-speed runs using each vessel's own position, not the raw df index
        run_id = (is_slow != is_slow.shift()).cumsum()
        slow = group[is_slow].copy()
        slow["_run"] = run_id[is_slow]
        for _, run in slow.groupby("_run"):
            if len(run) < 2:
                continue
